...
그림판이라는 클래스를 예로 들어보자.
그림판 클래스에는 도형을 저장해 저장한 도형들의 둘레와 넓이를 출력하는 메서드가 있다.
즉, 저장한 도형들 클래스 각각은 둘레와 넓이가 있다. 하지만 모든 도형이 그렇지는 않다.
예를 들어 삼각형이나 사각형 도형 클래스의 경우 둘레와 넓이가 있지만,
원기둥 같은 3d 도형에는 둘레와 넓이 메서드가 존재하지 않는다. 따라서 그림판 클래스의
저장 공간에 도형을 넣기 전에 해당 도형 클래스가 둘레와 넓이 메서드를 갖고 있는지 확인해야 한다.
이를  if문 isinstance(shape, Class) 를 통해 해결할 수 있다. 만약 Class가 사각형이면 true, 원기둥이면 false일 것이다. 
하지만 이럴 경우 새로운 도형이 추가되면 isinstance 문항을 무한정 추가해야 하는 문제가 생긴다.
따라서 차라리 둘레와 넓이가 존재하는 도형은 공통적인 부모 클래스를 상속받게끔 하고, 해당 클래스에 대해 isinstance를 적용하면 문제를 해결할 수 있다.
따라서 Shape이라는 부모 클래스를 만들어 빈 둘레와 넓이 메서드를 만든다. ( 비어두는 이유는 도형마다 둘레와 넓이 공식이 다르기 때문이다. 자식 클래스를 만들어 오버라이드를 한다.)

하지만 해당 부모 클래스를 상속해도 오버라이드를 하지 않으면 문제가 생길 것이다. 따라서 필수적으로 오버라이드를 하게 만드는 방법을 알아야 한다. 이는 추상 클래스로 해결한다.
추상 클래스란 여러 클래스들의 공통점을 추상화해서 모아놓은 클래스를 뜻한다.
이를 위해서는 
from abc import ABC, abstractmethod 를 불러온다. (abc는 abstract base class의 약자이다.)

Shape이라는 Class를 추상화하려면
먼저 앞서 import 한 ABC 클래스를 상속받게 한다. 그리고 필수로 만들고 싶은 메서드 위에 @abstractmethod를 한다.
class Shape(ABC):              
    @abstractmethod
    def area(self) -> float:
        pass
...
참고: 추상 클래스로 인스턴스를 만들 수 없다. 추상 클래스는 인스턴스를 만드는 목적으로 만들어진 것은 아니고 다른 클래스에 상속할 목적으로 만들어졌다.
def add_shape(self, shape: Shape):    ---> 이런 형태로 클래스 type hinting도 할 수 있다.

자식 클래스가 특정 변수를 갖도록 유도할 수 있다.
...
추상 클래스를 여러 개 다중 상속 받는 것은 일반적으로 많이 쓰인다.

클래스의 다형성 말고도 함수/메소드 다형성이 존재한다.

1. 옵셔널 파라미터란 기본 값을 미리 지정해준 파라미터다. (맨 뒤에 몰아서 써야 한다.)
2. *parameter  -> 정해지지 않은 갯수의 원소들을 튜플로 저장한다.

LBYL (Look Before You Leap)
-> if문으로 조건을 설정해서 에러를 방지하는 행위를 말한다. (어떤 작업을 수행하기 전에 그 작업을 수행해도 괜찮을지 확인)
